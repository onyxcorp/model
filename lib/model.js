...
var Model = function(attributes, options) {
    ...
    // model current ammount of changes
    this.revision = 1;
    // attributes object for this instance
    this.attributes = {};
...
lodash.objects.assign(Model.prototype, {
    ...
    // attributes that depends on other attributes data
    _dynamicAttributes: {},
    ...
    initialize: function () {
        if (!this._schema) {
            throw new Error('You forgot to set a _schema for the model, it is a required item');
        }
    },
    ...
    find: function (attr, data, first) {
        ...
        if (!lodash.objects.isNumber(attribute)) {
            ...
            throw new Error('Find can only be performed on String, Array or Object type attribute');
        }
    },
    ...
    // return the current model revision number
    ...
    get: function (attr) {
        if (this._schema.properties[attr] || this._dynamicAttributes[attr]) {
    ...
    set: function(key, val, options) {
        ...
        // verify if the model hasChanged at all from last update
        if (this.hasChanged()) {
            // if the model hasChanged we can update the revision
            this.revision += 1;

            // if we have dynamic attributes to set, update it
            if (!lodash.objects.isEmpty(this._dynamicAttributes)) {
                lodash.collections.forEach(this._dynamicAttributes, function (value, key) {
                    // key should be the attribute name, and value should be a function
                    if (typeof value !== 'function') {
                        throw new TypeError('DynamicAttributes must have a function as a value');
                    }
                    this.attributes[key] = value.apply(this);
                }.bind(this));
            }
        }
    ...
    checkSchema: function (attrs) {
        ...
        if (!this._schema) {
            throw new Error('You must set a _schema for this model, otherwise validate will always return false');
        } else { // check if the property being set set is correctly declared at the _schema attribute
            propertiesAndAttributes = lodash.objects.keys(this._schema.properties).concat(lodash.objects.keys(this._dynamicAttributes));
            difference = lodash.arrays.difference(lodash.objects.keys(attrs), propertiesAndAttributes);
            if (!lodash.objects.isEmpty(difference)) {
                throw new Error('You are trying to set attributes that are not described on the model _schema:' + difference.toString());
